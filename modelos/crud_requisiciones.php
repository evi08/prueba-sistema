...
switch ($_GET['op']) {
    ...
    case 'confirmar':
            ...
            while ($row = $resultado->fetch_assoc()) {
                ...
                VALUES ('$id_producto', $cantidad, $id_empleado, '$hora', 'SALIDA');";
                    if ($conexion->query($sql_kardex)) {
                        ...
                        echo "<script>alert('Error al insertar kardex')</script>";
                        echo "<script>setTimeout(\"location.href='../requisiciones'\",1)</script>";
                        die;
                    }
                } else {
                ...
                echo "<script>alert('Solicitud aprobada con éxito')</script>";
                echo "<script>setTimeout(\"location.href='../requisiciones'\",1)</script>";
                die;
        ...
        if ($estado == 3) {
            ...
            while ($row = $resultado->fetch_assoc()) {
                ...
                if ($conexion->query($sql)) {
                    ...
                    $sql = "INSERT INTO `tbl_kardex`(`id_producto`, `cantidad`, `cod_empleado`, `fecha_hora`, `tipo_movimiento`) 
                    ...
                    echo "<script>alert('Error al devolver inventario')</script>";
                    echo "<script>setTimeout(\"location.href='../requisiciones'\",1)</script>";
                    die;
                }
            }
        ...
        echo "<script>alert('Requisición devuelta con éxito')</script>";
        echo "<script>setTimeout(\"location.href='../requisiciones'\",1)</script>";
        die;
