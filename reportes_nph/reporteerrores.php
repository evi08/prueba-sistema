...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('               REPORTE DE ERRORES Y/O PROBLEMAS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(40, 12, utf8_decode("Id_Error"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Error"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Codigo"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Mensaje"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Fecha Lanzado"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 40, 80, 40, 60, 40)); //???
	}
...
if ($filtroerror != "") {
	$coincidenciaerror = "%".$filtroerror."%";
	$sqlerror = "SELECT * from tbl_errores
 	where id_error like '$coincidenciaerror' 
 	or error like '$coincidenciaerror' 
 	or codigo like '$coincidenciaerror'
	or mensaje like '$coincidenciaerror'
	or fecha_lanzado like '$coincidenciaerror'";
	$resultadoerror = $mysqli->query($sqlerror);
} else { 
	$sqlerror = "SELECT * FROM tbl_errores";
	$resultadoerror = $mysqli->query($sqlerror);
}	
...
while ($data = $resultadoerror->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_error']), utf8_decode($data['error']), utf8_decode($data['codigo']),utf8_decode($data['mensaje']),utf8_decode($data['fecha_lanzado'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportehogares.pdf');
