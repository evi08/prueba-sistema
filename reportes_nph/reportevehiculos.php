...
session_start();
$sqlbitacora = "SELECT * FROM tbl_objetos where id_objeto=61";
$resultadobitacora = $mysqli->query($sqlbitacora);

$rowbitacora = $resultadobitacora->fetch_assoc();
$id_objeto = $rowbitacora['id_objeto'];
$accion = "Solicitó reporte en el historial de vehiculos del sistema";
...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('         REPORTE DE VEHICULOS EXISTENTES '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(30, 12, utf8_decode("Cod_vehiculo"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Modelo Vehiculo"), 1, 0, 'C', 1);
        $this->Cell(60, 12, utf8_decode("Capacidad Personas"), 1, 0, 'C', 1);
        $this->Cell(65, 12, utf8_decode("Placa"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Descripcion"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 30, 50, 60, 65, 80)); //???
	}
...
if ($filtrovehiculo != "") {
	$coincidenciavehiculo = "%".$filtrovehiculo."%";
	$sqlvehiculos = "SELECT * from tbl_vehiculos
 	where cod_vehiculo like '$coincidenciavehiculo' 
 	or tipo_vehiculo like '$coincidenciavehiculo' 
 	or capacidad_personas like '$coincidenciavehiculo'
	or placa like '$coincidenciavehiculo'
	or descripcion like '$coincidenciavehiculo'";
	$resultadovehiculos = $mysqli->query($sqlvehiculos);
} else { 
	$sqlvehiculos = "SELECT * from tbl_vehiculos";
	$resultadovehiculos = $mysqli->query($sqlvehiculos);
}	
...
while ($data = $resultadovehiculos->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_vehiculo']), utf8_decode($data['tipo_vehiculo']), utf8_decode($data['capacidad_personas']), utf8_decode($data['placa']), utf8_decode($data['descripcion'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportevehiculos.pdf');
