...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('          REPORTE DE HISTORIAL DE PRODUCTOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(50, 12, utf8_decode("Id_Productos"), 1, 0, 'C', 1);
		$this->Cell(75, 12, utf8_decode("Nombre Producto"), 1, 0, 'C', 1);
		$this->Cell(70, 12, utf8_decode("Capacidad Minima"), 1, 0, 'C', 1);
		$this->Cell(70, 12, utf8_decode("Capacidad Maxima"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(35, 50, 75, 70, 70)); //???
	}
...
if ($filtroproducto!="") {
	$coincidenciaproducto = "%".$filtroproducto."%";
	$sqlproducto = "SELECT * from tbl_productos
 where id_producto like '$coincidenciaproducto' 
 or nombre_producto like '$coincidenciaproducto'
 or capacidad_min like '$coincidenciaproducto' 
 or capacidad_max like '$coincidenciaproducto'" ;
...
while ($data = $resultadoproducto->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_producto']), utf8_decode($data['nombre_producto']),utf8_decode($data['capacidad_min']),utf8_decode($data['capacidad_max'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
