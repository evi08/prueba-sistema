...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(118);
		$this->Cell(10, 20, utf8_decode('REPORTE DE HISTORIAL DE PRODUCTOS'));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->SetWidths(array(20, 40, 80, 70, 80)); //???
	}
...
if ($filtrofecha != "") {
	$coincidenciafecha = "%".$filtrofecha."%";
	$sqlfecha = "SELECT `tipo_movimiento`, `cantidad`, concat(e.primer_nombre, ' ', e.segundo_nombre, ' ', e.primer_apellido, ' ', e.segundo_apellido) empleado, `fecha_hora` 
	...
	$resultadofecha = $mysqli->query($sqlfecha);
} else { 
    $sqlfecha = "SELECT `tipo_movimiento`, `cantidad`, concat(e.primer_nombre, ' ', e.segundo_nombre, ' ', e.primer_apellido, ' ', e.segundo_apellido) empleado, `fecha_hora` 
	FROM tbl_kardex k, tbl_productos p, tbl_empleados e 
	WHERE p.id_producto = k.id_producto AND e.cod_empleado = k.cod_empleado AND p.nombre_producto = '$producto' 
...
while ($data = $resultadofecha->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cantidad']), utf8_decode($data['empleado']), utf8_decode($data['fecha_hora']), utf8_decode($data['tipo_movimiento'])) , 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
