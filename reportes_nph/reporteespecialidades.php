...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL DE ESPECIALIDADES DE EMPLEADOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(40, 12, utf8_decode("Id_Especialidad"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Especialidad Empleado"), 1, 0, 'C', 1);
		$this->Cell(150, 12, utf8_decode("Descripcion"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 40, 80, 150)); //???
	}
...
if ($filtroespecialidad!="") {
	$coincidenciacategoria = "%".$filtroespecialidad."%";
	$sqlespecialidad = "SELECT * from tbl_especialidades
	where cod_especialidad like '$coincidenciacategoria' 
	or nombre_especialidad like '$coincidenciacategoria' 
	or descripcion_especialidad like '$coincidenciacategoria'" ;
	$resultadoespecialidad = $mysqli->query($sqlespecialidad);
} else { 
	$sqlespecialidad = "SELECT `cod_especialidad`, `nombre_especialidad`, `descripcion_especialidad` 
...
while ($data = $resultadoespecialidad->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_especialidad']), utf8_decode($data['nombre_especialidad']), utf8_decode($data['descripcion_especialidad'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
