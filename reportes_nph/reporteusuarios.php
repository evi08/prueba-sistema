...
$filtrousuarios = $_POST['filtro_usuarios'];
...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(45, 12, utf8_decode("Rol"), 1, 0, 'C', 1);
        $this->Cell(60, 12, utf8_decode("Usuario"), 1, 0, 'C', 1);
        $this->Cell(60, 12, utf8_decode("Clave/encriptada"), 1, 0, 'C', 1);
        $this->Cell(55, 12, utf8_decode("Último login"), 1, 0, 'C', 1);
        $this->Cell(30, 12, utf8_decode("N° ingresos"), 1, 0, 'C', 1);
        $this->Cell(25, 12, utf8_decode("Estado"), 1, 1, 'C', 1); //salto de linea
        //El ancho de las celdas
        $this->SetWidths(array(20, 45, 60, 60, 55, 30, 25)); //???
    }
...
if ($filtrousuarios != "") {
    ...
    $sqlusuarios = "SELECT u.cod_usuario, r.rol, u.nombre_usuario_correo, u.clave_usuario, u.fecha_ultima_conexion, u.numero_ingresos, e.estado
    FROM tbl_usuarios_login u, tbl_roles_usuarios r, tbl_estado_usuarios e
    WHERE (r.id_rol = u.id_rol_usuario AND e.codigo_estado = u.cod_estado) and 
    (u.cod_usuario like '$coincidenciausuarios'
    or r.rol like '$coincidenciausuarios'
    or u.nombre_usuario_correo like '$coincidenciausuarios'
    or u.clave_usuario like '$coincidenciausuarios'
    or u.fecha_ultima_conexion like '$coincidenciausuarios'
    or u.numero_ingresos like '$coincidenciausuarios'
    or  e.estado like '$coincidenciausuarios')";
    $resultadousuarios = $mysqli->query($sqlusuarios);
} else {
    $sqlusuarios = "SELECT u.cod_usuario, r.rol, u.nombre_usuario_correo, u.clave_usuario, u.fecha_ultima_conexion, u.numero_ingresos, e.estado
    FROM tbl_usuarios_login u, tbl_roles_usuarios r, tbl_estado_usuarios e
    WHERE r.id_rol = u.id_rol_usuario AND e.codigo_estado = u.cod_estado;";
    $resultadousuarios = $mysqli->query($sqlusuarios);
}
...
while ($data = $resultadousuarios->fetch_assoc()) {
    ...
    $pdf->setX(15);
    $pdf->Row(array($i + 1, utf8_decode($data['rol']), utf8_decode($data['nombre_usuario_correo']),  utf8_decode($data['clave_usuario']), utf8_decode($data['fecha_ultima_conexion']), utf8_decode($data['numero_ingresos']), utf8_decode($data['estado'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
    $i = $i + 1;
}
