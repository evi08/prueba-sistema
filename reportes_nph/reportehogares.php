...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('               REPORTE DE HOGARES DE NPH '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(60, 12, utf8_decode("Id_Hogar"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Hogar"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Direccion"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Telefono"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 60, 80, 80, 60)); //???
	}
...
if ($filtrohogares!="") {
	$coincidenciahogares = "%".$filtrohogares."%";
	$sqlhogares = "SELECT * from tbl_hogares
 where id_hogar like '$coincidenciahogares' 
 or hogar like '$coincidenciahogares' 
 or direccion like '$coincidenciahogares' 
 or telefono like '$coincidenciahogares'" ;
$resultadohogares = $mysqli->query($sqlhogares);
} else { 
	$sqlhogares = "SELECT * from tbl_hogares";
$resultadohogares = $mysqli->query($sqlhogares);
}
...
while ($data = $resultadohogares->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_hogar']), utf8_decode($data['hogar']), utf8_decode($data['direccion']),utf8_decode($data['telefono'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
