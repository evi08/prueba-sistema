...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL DE CATEGORIA ACTIVOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(50, 12, utf8_decode("Cod_Categoria_Activo"), 1, 0, 'C', 1);
		$this->Cell(80, 12, utf8_decode("Nombre Categoria"), 1, 0, 'C', 1);
		$this->Cell(130, 12, utf8_decode("Descripcion"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(35, 50, 80, 130)); //???
	}
...
if ($filtrocategoria!="") {
	$coincidenciacategoria = "%".$filtrocategoria."%";
$sqlcategoria = "SELECT * from tbl_categoria_activos
 where cod_categoria_activo like '$coincidenciacategoria' 
 or nombre_categoria like '$coincidenciacategoria' 
 or descripcion like '$coincidenciacategoria'" ;
...
while ($data = $resultadocategoria->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_categoria_activo']), utf8_decode($data['nombre_categoria']),utf8_decode($data['descripcion'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportecategoria.pdf');
