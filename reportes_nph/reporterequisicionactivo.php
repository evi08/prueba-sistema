...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL DE REQUISICIONES DE ACTIVOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(50, 12, utf8_decode("Cod_Requisicion"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Id_Solicitud"), 1, 0, 'C', 1);
		$this->Cell(75, 12, utf8_decode("Nombre Hogar"), 1, 0, 'C', 1);
		$this->Cell(70, 12, utf8_decode("fecha Requisicion"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(35, 50, 50, 75, 70)); //???
	}
...
if ($filtrorequisiciona!="") {
	$coincidenciarequisiciona = "%".$filtrorequisiciona."%";
$sqlrequisiciona = "SELECT cod_requisicion, id_solicitud, hogar, fecha_ingreso 
FROM tbl_requisicion_activos, tbl_hogares 
where tbl_hogares.id_hogar = tbl_requisicion_activos.id_hogar AND 
 ( cod_requisicion like '$coincidenciarequisiciona' 
 or id_solicitud like '$coincidenciarequisiciona' 
 or hogar like '$coincidenciarequisiciona'
 or fecha_ingreso like '$coincidenciarequisiciona')" ;
$resultadorequisiciona = $mysqli->query($sqlrequisiciona);
} else { 
	$sqlrequisiciona = "SELECT cod_requisicion, id_solicitud, hogar, fecha_ingreso 
	FROM tbl_requisicion_activos, tbl_hogares 
...
while ($data = $resultadorequisiciona->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_requisicion']), utf8_decode($data['hogar']),utf8_decode($data['id_solicitud']),utf8_decode($data['fecha_ingreso'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reporterequisicionactivo.pdf');
