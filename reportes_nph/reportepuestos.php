...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('    REPORTE DE HISTORIAL DE PUESTOS DE EMPLEADOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(40, 12, utf8_decode("Id_Puesto"), 1, 0, 'C', 1);
		$this->Cell(70, 12, utf8_decode("Puesto Empleado"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Salario"), 1, 0, 'C', 1);
		$this->Cell(100, 12, utf8_decode("Funciones del Empleado"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 40, 70, 60, 100)); //???
	}
...
if ($filtropuesto != "") {
	$coincidenciapuesto = "%".$filtropuesto."%";
	$sqlpuesto = "SELECT * from tbl_puesto_empleados
 	where cod_puesto like '$coincidenciapuesto' 
 	or nombre_puesto like '$coincidenciapuesto' 
 	or salario_puesto like '$coincidenciapuesto'
	or funciones_puesto like '$coincidenciapuesto'";
	$resultadopuesto = $mysqli->query($sqlpuesto);
} else { 
	$sqlpuesto = "SELECT `cod_puesto`, `nombre_puesto`, `salario_puesto`, `funciones_puesto` 
   	FROM `tbl_puesto_empleados`;";
   	$resultadopuesto = $mysqli->query($sqlpuesto);
}	
...
while ($data = $resultadopuesto->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_puesto']), utf8_decode($data['nombre_puesto']), utf8_decode($data['salario_puesto']),utf8_decode($data['funciones_puesto'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportepuestos.pdf');
