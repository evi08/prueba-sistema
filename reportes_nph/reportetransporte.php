...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->Cell(30, 12, utf8_decode("Dpto"), 1, 0, 'C', 1);
		...
		$this->SetWidths(array(15, 15, 20, 30, 30, 30,30 ,30, 30,30,20)); //???
	}
...
if ($filtrotransporte != "") {
	...
	WHERE d.cod_departamento = s.cod_departamento  AND  em.cod_empleado = s.cod_empleado AND e.cod_estado = s.cod_estado AND s.id_tipo_solicitud = 5;";
	$resultadoplanitransporte = $mysqli->query($sqlplanitransporte);
} else { 
	$coincidenciatransporte = "%".$filtrotransporte."%";
	$sqlplanitransporte = "SELECT s.id_solicitud, s.id_tipo_solicitud,  d.nombre_departamento, concat(em.primer_nombre,' ', em.primer_apellido) empleado, s.fechahora_ingreso, s.fecha_fin, s.fecha_inicio, s.justificacion, s.opcion_permiso, e.nombre_del_estado
	FROM tbl_solicitudes s,  tbl_departamentos d, tbl_empleados em, tbl_estado_entrega_aprobacion e 
	WHERE d.cod_departamento = s.cod_departamento  AND  em.cod_empleado = s.cod_empleado AND e.cod_estado = s.cod_estado AND s.id_tipo_solicitud = 5 and 
	( s.id_solicitud like '$coincidenciatransporte' 
	or s.id_tipo_solicitud like '$coincidenciatransporte' 
	or d.nombre_departamento like '$coincidenciatransporte' 
	or em.primer_nombre like '$coincidenciatransporte' 
	or em.primer_apellido like '$coincidenciatransporte' 
	or s.fechahora_ingreso like '$coincidenciatransporte' 
	or s.fecha_fin like '$coincidenciatransporte' 
	or s.fecha_inicio like '$coincidenciatransporte' 
	or s.justificacion like '$coincidenciatransporte'
	or s.opcion_permiso like '$coincidenciatransporte'
	or e.nombre_del_estado like '$coincidenciatransporte')" ;
	$resultadoplanitransporte = $mysqli->query($sqlplanitransporte);
}
