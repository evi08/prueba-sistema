...
$coincidenciarequi = "%".$filtrorequi."%";
$sql = "SELECT s.id_solicitud, d.nombre_departamento, concat(em.primer_nombre,\" \",em.segundo_nombre,\" \",em.primer_apellido,\" \",em.segundo_apellido) empleado, h.hogar  , s.fechahora_ingreso, s.justificacion, e.nombre_del_estado 
	FROM tbl_solicitudes s, tbl_departamentos d, tbl_empleados em, tbl_hogares h, tbl_estado_entrega_aprobacion e, tbl_requisicion_activos ra 
	WHERE (d.cod_departamento = s.cod_departamento AND em.cod_empleado = s.cod_empleado AND h.id_hogar = ra.id_hogar AND e.cod_estado = s.cod_estado AND s.id_solicitud = ra.id_solicitud AND s.id_tipo_solicitud = 7 ) AND 
	s.id_solicitud like '$coincidenciarequi'
	or d.departamento like '$coincidenciarequi'
	or empleado like '$coincidenciarequi'
	or h.hogar like '$coincidenciarequi'
	or s.fechahora_ingreso like '$coincidenciarequi'
	or s.justificacion like '$coincidenciarequi'
	or e.nombre_del_estado  like '$coincidenciarequi'
	ORDER BY s.id_solicitud DESC;";
	$resultado = $mysqli->query($sql);

}
// ESTO SE DEBERIA DE BORRAR PERO SI LO HAGO TIRA ERROR EN LINEA 249//
$sql = "SELECT s.id_solicitud, d.nombre_departamento, concat(em.primer_nombre,\" \",em.segundo_nombre,\" \",em.primer_apellido,\" \",em.segundo_apellido) empleado, h.hogar  , s.fechahora_ingreso, s.justificacion, e.nombre_del_estado 
	FROM tbl_solicitudes s, tbl_departamentos d, tbl_empleados em, tbl_hogares h, tbl_estado_entrega_aprobacion e, tbl_requisicion_activos ra 
	WHERE d.cod_departamento = s.cod_departamento AND em.cod_empleado = s.cod_empleado AND h.id_hogar = ra.id_hogar AND e.cod_estado = s.cod_estado AND s.id_solicitud = ra.id_solicitud AND s.id_tipo_solicitud = 7 
	ORDER BY s.id_solicitud DESC;";
...
while ($data = $resultado->fetch_assoc()) {
	...
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_solicitud']), utf8_decode($data['nombre_departamento']), utf8_decode($data['empleado']), utf8_decode($data['hogar']), utf8_decode($data['fechahora_ingreso']), utf8_decode($data['justificacion']),  utf8_decode($data['nombre_del_estado'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
