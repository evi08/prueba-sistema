...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('           REPORTE DE INVENTARIO DE PRODUCTOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(90, 12, utf8_decode("Cod"), 1, 0, 'C', 1);
		$this->Cell(90, 12, utf8_decode("Producto"), 1, 0, 'C', 1);
		$this->Cell(90, 12, utf8_decode("Cantidad"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 90, 90, 90)); //???
	}
...
if ($filtroinventario!="") {
	$coincidenciainventario = "%".$filtroinventario."%";
$sqlinventario = "SELECT i.id_inventario, p.nombre_producto, i.cantidad
FROM tbl_inventarios i,tbl_productos p
WHERE i.id_producto = p.id_producto and 
 ( i.id_inventario like '$coincidenciainventario' 
 or p.nombre_producto like '$coincidenciainventario' 
 or i.cantidad like '$coincidenciainventario')" ;
$resultadoinventario = $mysqli->query($sqlinventario);
} else { 
	$sqlinventario = "SELECT i.id_inventario, p.nombre_producto, i.cantidad
FROM tbl_inventarios i,tbl_productos p
...
while ($data = $resultadoinventario->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_inventario']), utf8_decode($data['nombre_producto']), utf8_decode($data['cantidad'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reporteinventario.pdf');
