...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL Y/O TRANSACCIONES DEL SISTEMA'));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(30, 12, utf8_decode("Cod"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Objeto/pantalla"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Código de usuario"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Usuario/acción"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Fecha/hora acción"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Acción"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Descripción"), 1, 1, 'C', 1); //salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(15, 30, 40, 40, 60, 50, 40, 50)); //???
	}
...
if ($filtrobita != "") {
	$coincidenciabita = "%" . $filtrobita . "%";
	$sqlbitacora = "SELECT * from tbl_ms_bitacora
 	where id_bitacora like '$coincidenciabita' 
 	or id_objeto like '$coincidenciabita' 
 	or cod_usuario like '$coincidenciabita'
	or ejecutor like '$coincidenciabita'
	or fecha  like '$coincidenciabita'
	or accion like '$coincidenciabita'
	or descripcion like '$coincidenciabita'";
	$resultadobitacora = $mysqli->query($sqlbitacora);
} else {
	$sqlbitacora = "SELECT * from tbl_ms_bitacora";
	$resultadobitacora = $mysqli->query($sqlbitacora);
...
while ($data = $resultadobitacora->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_bitacora']), utf8_decode($data['id_objeto']), utf8_decode($data['cod_usuario']),  utf8_decode($data['ejecutor']), utf8_decode($data['fecha']), utf8_decode($data['accion']), utf8_decode($data['descripcion'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}
