...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('                    REPORTE DE COMPRAS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(40, 12, utf8_decode("Id_Compras"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Id_Solicitud"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Proveedor"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Fecha Compra"), 1, 0, 'C', 1);
		$this->Cell(60, 12, utf8_decode("Total a Pagar"), 1, 1, 'C', 1);//salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(20, 40, 40, 60,60, 60)); //???
	}
...
if ($filtrovercompra!="") {
	$coincidenciavercompra = "%".$filtrovercompra."%";
	...
	WHERE c.cod_proveedor = p.cod_proveedor AND s.id_solicitud = c.id_solicitud and 
	(c.id_compra like '$coincidenciavercompra' 
	or c.id_solicitud like '$coincidenciavercompra' 
	or p.nombre_proveedor like '$coincidenciavercompra'
	or c.fecha_compra like '$coincidenciavercompra'
	or c.total_pagar like '$coincidenciavercompra')" ;
...
while ($data = $resultado->fetch_assoc()) {
	...
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_compra']), utf8_decode($data['id_solicitud']), utf8_decode($data['nombre_proveedor']),utf8_decode($data['fecha_compra']),utf8_decode($data['total_pagar'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportevercompra.pdf');
