...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('       REPORTE DE PARÁMETROS DEL SISTEMA'));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(15, 12, utf8_decode("Cod"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Parámetro"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Valor"), 1, 0, 'C', 1);
		$this->Cell(30, 12, utf8_decode("Usuario"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Usuario creador"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Creación"), 1, 0, 'C', 1);
		$this->Cell(30, 12, utf8_decode("Modificado por"), 1, 0, 'C', 1);
		$this->Cell(40, 12, utf8_decode("Modificación"), 1, 1, 'C', 1); //Salto de linea
		//El ancho de las celdas
		$this->SetWidths(array(15, 15, 40, 50, 30, 40, 40, 30, 40)); //???
	}
...
if ($filtroparametro != "") {
	$coincidenciaparametro = "%".$filtroparametro."%";
	$sqlparametros = "SELECT * from tbl_parametros
 	where id_parametro like '$coincidenciaparametro' 
 	or parametro like '$coincidenciaparametro' 
 	or valor like '$coincidenciaparametro'
	or id_usuario like '$coincidenciaparametro'
	or creado_por like '$coincidenciaparametro' 
 	or fecha_creacion like '$coincidenciaparametro'
	or modificado_por like '$coincidenciaparametro'
	or fecha_modificacion like '$coincidenciaparametro'";
	$resultado = $mysqli->query($sqlparametros);
} else { 
	$sqlparametros = "SELECT * from tbl_parametros";
	$resultado = $mysqli->query($sqlparametros);
}	
...
while ($data = $resultado->fetch_assoc()) {
	...
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_parametro']), utf8_decode($data['parametro']), utf8_decode($data['valor']), utf8_decode($data['id_usuario']), utf8_decode($data['creado_por']), utf8_decode($data['fecha_creacion']), utf8_decode($data['modificado_por']), utf8_decode($data['fecha_modificacion'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'ReporteParametros.pdf');
