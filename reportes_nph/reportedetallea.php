...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL DE DETALLE DE REQUISICIONES ACTIVOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(35, 12, 'Num', 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Id_Detalle"), 1, 0, 'C', 1);
		$this->Cell(55, 12, utf8_decode("Solicitante"), 1, 0, 'C', 1);
		$this->Cell(55, 12, utf8_decode("Producto"), 1, 0, 'C', 1);
		$this->Cell(55, 12, utf8_decode("Cantidad"), 1, 1, 'C', 1);//salto de linea
		
		//El ancho de las celdas
		$this->SetWidths(array(35, 50, 55, 55, 55 )); //???
	}
...
if ($filtrodetaller!="") {
	$coincidenciadetaller = "%".$filtrodetaller."%";
	$sqldetallea = "SELECT id_detalle_requisicion, cod_requisicion, tbl_productos.nombre_producto, cantidad
	FROM nph.tbl_detalle_requisiciones, tbl_productos 
	where tbl_detalle_requisiciones.id_producto = tbl_productos.id_producto AND
 ( id_detalle_requisicion like '$coincidenciadetaller' 
 or cod_requisicion like '$coincidenciadetaller' 
 or tbl_productos.nombre_producto like '$coincidenciadetaller'
 or cantidad like '$coincidenciadetaller')" ;
$resultadodetallea = $mysqli->query($sqldetallea);
} else { 
...
while ($data = $resultadodetallea->fetch_assoc()) {
	$pdf->SetFont('Arial', '', 10);
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['id_detalle_requisicion']), utf8_decode($data['cod_requisicion']),utf8_decode($data['nombre_producto']),utf8_decode($data['cantidad'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reportedetalleactivo.pdf');
