...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->SetY(30);
		$this->SetX(100);
		$this->Cell(10, 10, utf8_decode('REPORTE DE HISTORIAL DE ACTIVOS '));
		$this->SetX(5);
		$this->Ln(11);
		...
		$this->Cell(30, 12, utf8_decode("Cod_Activo"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Categoria Activo"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Proveedor"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Nombre Activo"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Descripcion Activo"), 1, 0, 'C', 1);
		$this->Cell(50, 12, utf8_decode("Presentacion Activo"), 1, 1, 'C', 1);//salto de linea
...
if ($filtroactivo!="") {
	$coincidenciaactivo = "%".$filtroactivo."%";
$sqlactivo = "SELECT a.cod_activo, c.nombre_categoria, p.nombre_proveedor, a.nombre_activo, a.descripcion_activo, a.presentacion_activo 
FROM tbl_activos a, tbl_categoria_activos c, tbl_proveedores p 
WHERE( c.cod_categoria_activo = a.cod_categoria_activo AND p.cod_proveedor = a.cod_proveedor_activo) and 
 ( a.cod_activo like '$coincidenciaactivo' 
 or c.nombre_categoria like '$coincidenciaactivo' 
 or p.nombre_proveedor like '$coincidenciaactivo'
 or a.nombre_activo like '$coincidenciaactivo'
 or a.descripcion_activo like '$coincidenciaactivo'
 or a.presentacion_activo  like '$coincidenciaactivo')" ;
$resultado = $mysqli->query($sqlactivo);
} else { 
	$sqlactivo = "SELECT a.cod_activo, c.nombre_categoria, p.nombre_proveedor, a.nombre_activo, a.descripcion_activo, a.presentacion_activo 
	FROM tbl_activos a, tbl_categoria_activos c, tbl_proveedores p 
...
while ($data = $resultado->fetch_assoc()) {
	...
	$pdf->setX(15);
	$pdf->Row(array($i + 1, utf8_decode($data['cod_activo']), utf8_decode($data['nombre_categoria']), utf8_decode($data['nombre_proveedor']), utf8_decode($data['nombre_activo']), utf8_decode($data['descripcion_activo']), utf8_decode($data['presentacion_activo'])), 25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
	$i = $i + 1;
}

// cell(ancho, largo, contenido,borde?, salto de linea?)

$pdf->Output('I', 'Reporteactivo.pdf');
