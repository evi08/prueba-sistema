...
<?php
    ...
    if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
        ...
        $sqlgestionerrores = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=56";
        $resultadogestionerrores = $mysqli->query($sqlgestionerrores );
        $filasgestionerrores = $resultadogestionerrores->num_rows;

        if ($filasgestionerrores  > 0) {
            $rowgestionerrores = $resultadogestionerrores->fetch_assoc();
            $permisoconsultagestionerrores = $rowgestionerrores['permiso_consultar'];
            if ($permisoconsultagestionerrores == 1) {
	$id = $_SESSION['id'];
	...
	$sql = "SELECT * FROM tbl_errores  $where";
    $resultadoerror = $mysqli->query($sql);
	...
	<div class="content-wrapper">
		...
		<!-- /.content-header -->
		<form action="reportes_nph/reporteerrores.php" id="frm_enviar_filtro_errores" method="post">
						<input type="hidden" name="filtro_error" id="filtro_error">
						<div>
							<button id="btn_formulario_errores" class="btn btn-danger" name="btn_formulario_errores" target="_blank" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de Errores</button>
						</div>
					</form>
                    <script>
						$('#btn_formulario_errores').on('click', function() {

							var filtro = $('#inventario_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_error').val(filtro);

							console.log('#filtro_error');
							console.log($('#filtro_error').val());
							document.getElementById('frm_enviar_filtro_errores').submit();
		...
		<div>
			<div class="card-body" style="overflow-x:auto;">
				<div class="table-responsive">
					...
					<thead>
						...
						<tbody>
							<?php while ($row_error = $resultadoerror->fetch_assoc()) { ?>
								<tr>
								
									
									<td><?php echo $row_error['id_error']; ?></td>
									<td><?php echo $row_error['error']; ?></td>
									<td><?php echo $row_error['codigo']; ?></td>
									<td><?php echo $row_error['mensaje']; ?></td>
									<td><?php echo $row_error['fecha_lanzado']; ?></td>
			...
			<div class="modal fade" id="filtroerror" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="reportes_nph/reporteerrores.php" method="post">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtración</b> </label><input class="form-control py-4" id="filtroempleados" type="text" name="filtroerror" placeholder="Ingrese una coincidencia" maxlength=""  /></div>
                                                </div>
