...
<head>
	...
	<title>Detalle de Requisiciones</title>
...
<body>
	...
	if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
		...
		$sqlgestiondetallea = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=51";
		$resultadogestiondetallea = $mysqli->query($sqlgestiondetallea);
		$filasgestiondetallea = $resultadogestiondetallea->num_rows;

		if ($filasgestiondetallea  > 0) {
			$rowgestiondetallea = $resultadogestiondetallea->fetch_assoc();
			$permisoconsultagestiondetallea = $rowgestiondetallea['permiso_consultar'];
			if ($permisoconsultagestiondetallea == 1) {
				$id = $_SESSION['id'];
	...
	WHERE ed.cod_empleado = u.cod_empleado AND d.cod_departamento = ed.cod_departamento AND e.cod_empleado = u.cod_empleado AND u.cod_usuario = $id;";
				...
				//$sql = "SELECT * FROM tbl_detalle_requisiciones $where";
				$sql = "SELECT id_detalle_requisicion, cod_requisicion, tbl_productos.nombre_producto, cantidad
	FROM nph.tbl_detalle_requisiciones, tbl_productos 
	where tbl_detalle_requisiciones.id_producto = tbl_productos.id_producto;";
				$resultado = $mysqli->query($sql);
				...
				<div class="content-wrapper">
					...
					<div class="content-header">
						<div class="container-fluid">
							<div class="row mb-2">
								<div class="col-sm-6">
									<h1 class="m-0">Detalle de Requisiciones</h1>
								</div><!-- /.col -->
								<div class="col-sm-6">
									<ol class="breadcrumb float-sm-right">
										<li class="breadcrumb-item"><a href="#">NPH</a></li>
										<li class="breadcrumb-item active">Detalle de Requisiciones</li>
									</ol>
								</div><!-- /.col -->
							</div><!-- /.row -->
						</div><!-- /.container-fluid -->
					...
					<!-- /.content-header -->
					<form action="reportes_nph/reportedetallea.php" id="frm_enviar_filtro_detallea" method="post">
						<input type="hidden" name="filtro_detallea" id="filtro_detallea">
						<div>
							<button id="btn_formulario_detallea" class="btn btn-danger" name="btn_formulario_detallea" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte listad de requisiciones de activos</button>
						</div>
					</form>
					<script>
						$('#btn_formulario_detallea').on('click', function() {

							var filtro = $('#detallea_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_detallea').val(filtro);

							console.log('#filtro_detallea');
							console.log($('#filtro_detallea').val());
							document.getElementById('frm_enviar_filtro_detallea').submit();
					...
					<div>
						<div class="card-body" style="overflow-x:auto;">
							<div class="table-responsive">
								<table id="detallea" class="table table-bordered ">
									...
									<tbody>
										<?php while ($row_kardex = $resultado->fetch_assoc()) { ?>
											<tr>
												...
												<td><?php echo $row_kardex['id_detalle_requisicion']; ?></td>
												<td><?php echo $row_kardex['cod_requisicion']; ?></td>
												<td><?php echo $row_kardex['nombre_producto']; ?></td>
												<td><?php echo $row_kardex['cantidad']; ?></td>
						...
						<div class="modal fade" id="filtrodetaller" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<form action="reportes_nph/reportedetallea.php" method="post">
											...
											<div class="form-group">
												...
												<div class="col-md-8">
													<div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtraciòn</b> </label><input class="form-control py-4" id="filtroadetaller" type="text" name="filtrodetaller" placeholder="Ingrese una coincidencia" maxlength="" /></div>
												</div>
