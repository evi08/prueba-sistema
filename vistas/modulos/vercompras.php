...
<?php
    ...
    if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
        ...
        $sqlgestionvercompras = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=53";
        $resultadogestionvercompras = $mysqli->query($sqlgestionvercompras );
        $filasgestionvercompras = $resultadogestionvercompras->num_rows;

        if ($filasgestionvercompras  > 0) {
            $rowgestionvercompras = $resultadogestionvercompras->fetch_assoc();
            $permisoconsultagestionvercompras = $rowgestionvercompras['permiso_consultar'];
            if ($permisoconsultagestionvercompras == 1) {
	$id = $_SESSION['id'];
	...
	$sql = "SELECT c.id_compra, c.id_solicitud, p.nombre_proveedor, c.fecha_compra, c.total_pagar 
	FROM tbl_compras c, tbl_proveedores p, tbl_solicitudes s 
	WHERE c.cod_proveedor = p.cod_proveedor AND s.id_solicitud = c.id_solicitud;";
    $resultado = $mysqli->query($sql);
	...
	<div class="content-wrapper">
		...
		<!-- /.content-header -->
		<form action="reportes_nph/reportevercompras.php" id="frm_enviar_vercompras" method="post">
						<input type="hidden" name="filtrovercompra" id="input_vercompras">
						<div>
							<button id="btn_enviar_formulario" class="btn btn-danger" name="btnreportedepartamento" target="_blank" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de ver compras</button>
						</div>
					</form>
                    <script>
						$('#btn_enviar_formulario').on('click', function() {

							var filtro = $('#vercompras_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#input_vercompras').val(filtro);

							console.log('#input_vercompras');
							console.log($('#input_vercompras').val());
							document.getElementById('frm_enviar_vercompras').submit();
                        });
                        </script>
		<div>
		<div>
			<div class="card-body" style="overflow-x:auto;">
				<div class="table-responsive">
					<table id="vercompras" class="table table-bordered ">
						...
						<tbody>
							<?php while ($row_compras = $resultado->fetch_assoc()) { ?>
								<tr>
									
									<td><?php echo $row_compras['id_compra']; ?></td>
									<td><?php echo $row_compras['id_solicitud']; ?></td>
									<td><?php echo $row_compras['nombre_proveedor']; ?></td>
									<td><?php echo $row_compras['fecha_compra']; ?></td>
									<td><?php echo $row_compras['total_pagar']; ?></td>
			...
			<div class="modal fade" id="filtrovercompra" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="reportes_nph/reportevercompras.php" method="post">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtración</b> </label><input class="form-control py-4" id="filtroempleados" type="text" name="filtrovercompra" placeholder="Ingrese una coincidencia" maxlength=""  /></div>
                                                </div>
								...
								<div class="form-group">
									...
									<select class="form-control" id="categoria_editar" name="categoria" required>
										...
										while ($row = $resultado->fetch_assoc()) { ?>
											<option value=<?php echo $row['cod_categoria_activo']; ?>> <?php echo $row['nombre_categoria'];} ?></option>
									</select>
								</div>
...
-->
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			var table = $('#vercompras').DataTable({
				orderCellsTop: true,
				fixedHeader: true,
				language: {
