...
<?php
    ...
    if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
        ...
        $sqlgestionproducto = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=48";
        $resultadogestionproducto = $mysqli->query($sqlgestionproducto );
        $filasgestionproducto = $resultadogestionproducto->num_rows;

        if ($filasgestionproducto  > 0) {
            $rowgestionproducto = $resultadogestionproducto->fetch_assoc();
            $permisoinserciongestionproducto = $rowgestionproducto['permiso_insercion'];
            $permisoeliminaciongestionproducto = $rowgestionproducto['permiso_eliminacion'];
            $permisoactualizaciongestionproducto = $rowgestionproducto['permiso_actualizacion'];
            $permisoconsultagestionproducto = $rowgestionproducto['permiso_consultar'];
            if ($permisoconsultagestionproducto == 1) {
	$id = $_SESSION['id'];
	...
	$sql = "SELECT * FROM tbl_productos  $where";
    $resultado = $mysqli->query($sql);
	...
	<div class="content-wrapper">
		...
		<!-- /.content-header -->
		<form action="reportes_nph/reporteproducto.php" id="frm_enviar_filtro_productos" method="post">
						<input type="hidden" name="filtro_productos" id="filtro_productos">
						<div>
							<button id="btn_formulario_producto" class="btn btn-danger" name="btn_formulario_producto" target="_blank" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de Productos</button>
						</div>
					</form>
                    <script>
						$('#btn_formulario_producto').on('click', function() {

							var filtro = $('#usuarios_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_productos').val(filtro);

							console.log('#filtro_productos');
							console.log($('#filtro_productos').val());
							document.getElementById('frm_enviar_filtro_productos').submit();
			...
			<div class="card-body" style="overflow-x:auto;">
				<div class="table-responsive">
					<table id="usuarios" class="table table-bordered ">
						...
						<tbody>
							<?php while ($row_productos = $resultado->fetch_assoc()) { ?>
								<tr>
									<td>
										<?php if ($permisoactualizaciongestionproducto == 1) { ?>
											<!-- BOTTON EDITAR -->
											<div class="col-md-6">
											...
											<?php } ?>
											<!-- BOTTON ELIMINAR -->
											<?php if ($permisoeliminaciongestionproducto == 1) { ?>
											<div class="col-md-6">
									...
									<td><?php echo $row_productos['id_producto']; ?></td>
									<td><?php echo $row_productos['nombre_producto']; ?></td>
									<td><?php echo $row_productos['capacidad_min']; ?></td>
									<td><?php echo $row_productos['capacidad_max']; ?></td>
									
									
								</tr>
							<?php } ?>

							<?php if ($permisoinserciongestionproducto == 1) { ?>
								<h1 class="box-title">
...
<div class="modal fade" id="filtroproducto" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="reportes_nph/reporteproducto.php" method="post">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtraciòn</b> </label><input class="form-control py-4" id="filtroproducto" type="text" name="filtroproducto" placeholder="Ingrese una coincidencia" maxlength=""  /></div>
                                                </div>
			...
			<div class="modal fade" id="modalEliminar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form action="modelos/crud_productos.php?op=eliminar" method="post">

								<div class="form-group">
								<label for="">¿Seguro quieres eliminar Producto </label>
									<label id="idEliminarLabel"></label>
								</div>
			...
			<div class="modal fade" id="modalEditar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form action="modelos/crud_productos.php?op=editar" method="post">
								...
								<!-- FORMULARIO
								$('#nombreproducto_editar').val(datos[2]);
			$('#capacidadmin_editar').val(datos[3]);
			$('#capacidadmax_editar').val(datos[4]); --> 

								<div class="form-group">
									<label class="small mb-1" for="activo">Ingrese un Producto</label>
									<input class="form-control" id="nombreproducto_editar" type="text" name="producto" onKeyUP="this.value=this.value.toUpperCase();" onkeypress="return evitarespeciales(event);" value=""  placeholder="Ingrese Producto" maxlength="15" required/>
								</div>

								<div class="form-group">
									<label class="small mb-1" for="descripcion">Capacidad Minima</label>
									<input class="form-control" id="capacidadmin_editar" type="number" name="capacidadmin" value=""  placeholder="Capacidad Minima" maxlength="30" required/>
								</div>
								<div class="form-group">
									<label class="small mb-1" for="descripcion">Capacidad Maxima</label>
									<input class="form-control" id="capacidadmax_editar" type="number" name="capacidadmax" value=""  placeholder="Capacidad Maxima" maxlength="30" required/>
								</div>
			...
			<div class="modal fade" id="modalAñadir" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form action="modelos/crud_productos.php?op=añadir" method="post">

								<!-- FORMULARIO -->

								<div class="form-group">
									<label class="small mb-1" for="activo">Ingrese un Producto</label>
									<input class="form-control" id="activo" type="text" name="producto" onKeyUP="this.value=this.value.toUpperCase();" onkeypress="return evitarespeciales(event);"value=""  placeholder="Ingrese Producto" maxlength="15" required/>
								</div>

								<div class="form-group">
									<label class="small mb-1" for="descripcion">Capacidad Minima</label>
									<input class="form-control" id="descripcion_editar" type="number" name="capacidadmin" value=""  placeholder="Capacidad Minima" maxlength="30" required/>
								</div>
								<div class="form-group">
									<label class="small mb-1" for="descripcion">Capacidad Maxima</label>
									<input class="form-control" id="descripcion_editar" type="number" name="capacidadmax" value=""  placeholder="Capacidad Maxima" maxlength="30" required/>
								</div>
