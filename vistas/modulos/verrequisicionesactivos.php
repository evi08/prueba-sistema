...
<body>
	...
	if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
		...
		$sqlgestionrequisia = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=50";
		$resultadogestionrequisia = $mysqli->query($sqlgestionrequisia);
		$filasgestionrequisia = $resultadogestionrequisia->num_rows;

		if ($filasgestionrequisia  > 0) {
			$rowgestionrequisia = $resultadogestionrequisia->fetch_assoc();
			$permisoconsultagestionrequisia = $rowgestionrequisia['permiso_consultar'];
			if ($permisoconsultagestionrequisia == 1) {
				$id = $_SESSION['id'];
	...
	WHERE ed.cod_empleado = u.cod_empleado AND d.cod_departamento = ed.cod_departamento AND e.cod_empleado = u.cod_empleado AND u.cod_usuario = $id;";
				...
				$sql = "SELECT cod_requisicion, id_solicitud, hogar, fecha_ingreso 
	FROM tbl_requisicion_activos, tbl_hogares 
	where tbl_hogares.id_hogar = tbl_requisicion_activos.id_hogar;";
				$resultado = $mysqli->query($sql);
				...
				<div class="content-wrapper">
					...
					<div class="content-header">
						<div class="container-fluid">
							<div class="row mb-2">
								<div class="col-sm-6">
									<h1 class="m-0">Requisiciones de Activos</h1>
								</div><!-- /.col -->
								<div class="col-sm-6">
									<ol class="breadcrumb float-sm-right">
										<li class="breadcrumb-item"><a href="#">NPH</a></li>
										<li class="breadcrumb-item active">Requisiciones Activos</li>
									</ol>
								</div><!-- /.col -->
							</div><!-- /.row -->
						</div><!-- /.container-fluid -->
					...
					<!-- /.content-header -->
					<form action="reportes_nph/reporterequisicionactivo.php" id="frm_enviar_filtro_requiactivos" method="post">
						<input type="hidden" name="filtro_requiactivos" id="filtro_requiactivos">
						<div>
							<button id="btn_formulario_requiactivos" class="btn btn-danger" name="btn_formulario_requiactivos" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte listad de requisiciones de activos</button>
						</div>
					</form>
					<script>
						$('#btn_formulario_requiactivos').on('click', function() {

							var filtro = $('#requiactivos_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_requiactivos').val(filtro);

							console.log('#filtro_requiactivos');
							console.log($('#filtro_requiactivos').val());
							document.getElementById('frm_enviar_filtro_requiactivos').submit();
					...
					<div>
						<div class="card-body" style="overflow-x:auto;">
							<div class="table-responsive">
								<table id="requiactivos" class="table table-bordered ">
									...
									<tbody>
										<?php while ($row_kardex = $resultado->fetch_assoc()) { ?>
											<tr>
												...
												<td><?php echo $row_kardex['id_solicitud']; ?></td>
												<td><?php echo $row_kardex['hogar']; ?></td>
												<td><?php echo $row_kardex['fecha_ingreso']; ?></td>
											</tr>
										<?php } ?>
						...
						<div class="modal fade" id="filtrorequisiciona" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<form action="reportes_nph/reporterequisicionactivo.php" method="post">
											...
											<div class="form-group">
												...
												<div class="col-md-8">
													<div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtraciòn</b> </label><input class="form-control py-4" id="filtroactivo" type="text" name="filtrorequisiciona" placeholder="Ingrese una coincidencia" maxlength="" /></div>
												</div>
...
-->
				...
				<script type="text/javascript">
					...
					$(document).ready(function() {
						var table = $('#requiactivos').DataTable({
							orderCellsTop: true,
							fixedHeader: true,
							language: {
