...
<?php
    ...
    if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
        ...
        $sqlgestiondetallecompra = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=54";
        $resultadogestiondetallecompra = $mysqli->query($sqlgestiondetallecompra );
        $filasgestiondetallecompra = $resultadogestiondetallecompra->num_rows;

        if ($filasgestiondetallecompra  > 0) {
            $rowgestiondetallecompra = $resultadogestiondetallecompra->fetch_assoc();
            $permisoconsultagestiondetallecompra = $rowgestiondetallecompra['permiso_consultar'];
            if ($permisoconsultagestiondetallecompra == 1) {
	$id = $_SESSION['id'];
	...
	$sql = "SELECT d.id_detalle_compra, d.id_compra, p.nombre_producto, d.cantidad, d.precio_compra 
	FROM tbl_detalle_compra d, tbl_productos p 
	...
	<div class="content-wrapper">
		...
		<div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">Detalle Compra</h1>
					</div><!-- /.col -->
					<div class="col-sm-6">
		...
		<!-- /.content-header -->
		<form action="reportes_nph/reportedetallecompras.php" id="frm_enviar_filtro_detallesc" method="post">
			<input type="hidden" name="filtro_detalles" id="filtro_detalles">
			<div>
				<button id="btn_formulario_detallecom" class="btn btn-danger" name="btn_formulario_detallecom" target="_blank" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de Detalle Compra</button>
			</div>
		</form>
        <script>
			$('#btn_formulario_detallecom').on('click', function() {

				var filtro = $('#usuarios_filter > label > input[type=search]').val();
				console.log(filtro);
				$('#filtro_detalles').val(filtro);

				console.log('#filtro_detalles');
				console.log($('#filtro_detalles').val());
				document.getElementById('frm_enviar_filtro_detallesc').submit();
		...
		<div>
			<div class="card-body" style="overflow-x:auto;">
				<div class="table-responsive">
					<table id="usuarios" class="table table-bordered ">
						...
						<tbody>
							<?php while ($row_detalleCompra = $resultado->fetch_assoc()) { ?>
								<tr>
									
									<td><?php echo $row_detalleCompra['id_detalle_compra']; ?></td>
									<td><?php echo $row_detalleCompra['id_compra']; ?></td>
									<td><?php echo $row_detalleCompra['nombre_producto']; ?></td>
									<td><?php echo $row_detalleCompra['cantidad']; ?></td>
									<td><?php echo $row_detalleCompra['precio_compra']; ?></td>
			...
			<div class="modal fade" id="filtrodetallec" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="reportes_nph/reportedetallecompras.php" method="post">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtraciòn</b> </label><input class="form-control py-4" id="filtrodetallec" type="text" name="filtrodetallec" placeholder="Ingrese una coincidencia" maxlength=""  /></div>
                                                </div>
