...
<body>
	...
	if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
		...
		$sqlgestioncategoria = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=44";
		$resultadogestioncategoria = $mysqli->query($sqlgestioncategoria);
		$filasgestioncategoria = $resultadogestioncategoria->num_rows;

		if ($filasgestioncategoria  > 0) {
			$rowgestioncategoria = $resultadogestioncategoria->fetch_assoc();
			$permisoinserciongestioncategoria = $rowgestioncategoria['permiso_insercion'];
			$permisoeliminaciongestioncategoria = $rowgestioncategoria['permiso_eliminacion'];
			$permisoactualizaciongestioncategoria = $rowgestioncategoria['permiso_actualizacion'];
			$permisoconsultagestioncategoria = $rowgestioncategoria['permiso_consultar'];
			if ($permisoconsultagestioncategoria == 1) {
				$id = $_SESSION['id'];
	...
	WHERE ed.cod_empleado = u.cod_empleado AND d.cod_departamento = ed.cod_departamento AND e.cod_empleado = u.cod_empleado AND u.cod_usuario = $id;";
				...
				$sql = "SELECT * FROM tbl_categoria_activos  $where";
				$resultado = $mysqli->query($sql);
				...
				<div class="content-wrapper">
					...
					<div class="content-header">
						<div class="container-fluid">
							<div class="row mb-2">
								<div class="col-sm-6">
									<h1 class="m-0">Categoria Activos</h1>
								</div><!-- /.col -->
								<div class="col-sm-6">
									<ol class="breadcrumb float-sm-right">
										<li class="breadcrumb-item"><a href="#">NPH</a></li>
										<li class="breadcrumb-item active">Categoria Activos</li>
									</ol>
								</div><!-- /.col -->
							</div><!-- /.row -->
						</div><!-- /.container-fluid -->
					...
					<!-- /.content-header -->
					<form action="reportes_nph/reportecategoria.php" id="frm_enviar_filtro_categorias" method="post">
						<input type="hidden" name="filtro_categorias" id="filtro_categorias">
						<div>
							<button id="btn_formulario_categorias" class="btn btn-danger" name="btn_formulario_categorias" target="_blank" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de categorias</button>
						</div>
					</form>
					<script>
						$('#btn_formulario_categorias').on('click', function() {

							var filtro = $('#categorias_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_categorias').val(filtro);

							console.log('#filtro_categorias');
							console.log($('#filtro_categorias').val());
							document.getElementById('frm_enviar_filtro_categorias').submit();
							...
							<div class="card-body" style="overflow-x:auto;">
								<div class="table-responsive">
									<table id="categorias" class="table table-bordered ">
										...
										<tbody>
											<?php while ($row_categoria = $resultado->fetch_assoc()) { ?>
												<tr>
													<td>
														<?php if ($permisoactualizaciongestioncategoria == 1) { ?>
															<!-- BOTTON EDITAR -->
															<div class="col-md-6">
														...
														<?php } ?>
														<!-- BOTTON ELIMINAR -->
														<?php if ($permisoeliminaciongestioncategoria == 1) { ?>
															<div class="col-md-6">
													...
													<td><?php echo $row_categoria['cod_categoria_activo']; ?></td>
													<td><?php echo $row_categoria['nombre_categoria']; ?></td>
													<td><?php echo $row_categoria['descripcion']; ?></td>

												</tr>
											<?php } ?>

											<?php if ($permisoinserciongestioncategoria == 1) { ?>
												<h1 class="box-title">
													...
													<button type="button" id="btnAñadir" class="btn btn-success btnAñadir" style="background:dodgerblue" data-bs-toggle="modal" data-bs-target="#modalAñadir">
														<i class="fa fa-plus-circle"></i>&nbsp; Nueva Categoria Activo
													</button>
												</h1>
											<?php } ?>
										</tbody>
										...
										<div class="modal-body">
											<form action="modelos/crud_categoriaactivo.php?op=editar" method="post">
												...
												<div class="form-group">
													...
													<input class="form-control" id="id_categoria_editar" type="text" name="id_categoria_editar" value="" maxlength="30" required readonly />
												</div>

												<div class="form-group">
													<label class="small mb-1" for="descripcion">Edite nombre de categoria</label>
													<input class="form-control" id="categoria_editar" type="text" name="categoria_editar" onKeyUP="this.value=this.value.toUpperCase();" onkeypress="return evitarespeciales(event);" value="" placeholder="Ingrese Categoria Activo" maxlength="30" required />
												</div>
										...
										<div class="modal-body">
											<form action="modelos/crud_categoriaactivo.php?op=añadir" method="post">
												...
												<div class="form-group">
													...
													<input class="form-control" id="activo" type="text" name="activo" onKeyUP="this.value=this.value.toUpperCase();" onkeypress="return evitarespeciales(event);" value="" placeholder="Ingrese Categoria Activo" maxlength="15" required />
												</div>
...
-->
					...
					<script type="text/javascript">
						...
						$('.btnEditar').on('click', function() {
							...
							$('#id_categoria_editar').val(datos[1]);
