...
<body>
	...
	if (!isset($_SESSION['session']) && $_SESSION['session'] != 'ok') {
		...
		$sqlgestioninventario = "SELECT * FROM tbl_permisos WHERE id_rol='$codrolusuario' AND id_objeto=45";
		$resultadogestioninventario = $mysqli->query($sqlgestioninventario);
		$filasgestioninventario = $resultadogestioninventario->num_rows;

		if ($filasgestioninventario  > 0) {
			$rowgestioninventario = $resultadogestioninventario->fetch_assoc();
			$permisoconsultagestioninventario = $rowgestioninventario['permiso_consultar'];
			if ($permisoconsultagestioninventario == 1) {
				$id = $_SESSION['id'];
	...
	WHERE ed.cod_empleado = u.cod_empleado AND d.cod_departamento = ed.cod_departamento AND e.cod_empleado = u.cod_empleado AND u.cod_usuario = $id;";
				...
				$sql = "SELECT i.id_inventario, p.nombre_producto, i.cantidad
    FROM tbl_inventarios i,tbl_productos p
	...
	?>
				...
				<div class="content-wrapper">
					...
					<!-- /.content-header -->
					<form action="reportes_nph/reporteinventario.php" id="frm_enviar_filtro_inventario" method="post">
						<input type="hidden" name="filtro_inventario" id="filtro_inventario">
						<div>
							<button id="btn_formulario_inventario" class="btn btn-danger" name="btn_formulario_inventario" style="border:20px;margin: 20px;" type="button"><i class="nav-icon fas fa-file-pdf"></i> Reporte de inventarios</button>
						</div>
					</form>
					<script>
						$('#btn_formulario_inventario').on('click', function() {

							var filtro = $('#inventario_filter > label > input[type=search]').val();
							console.log(filtro);
							$('#filtro_inventario').val(filtro);

							console.log('#filtro_inventario');
							console.log($('#filtro_inventario').val());
							document.getElementById('frm_enviar_filtro_inventario').submit();
					...
					<div>
						<div class="card-body" style="overflow-x:auto;">
							<div class="table-responsive">
								<table id="inventario" class="table table-bordered ">
									...
									<tbody>
										<?php while ($row_inventario = $resultado->fetch_assoc()) { ?>
											<tr>
												<td>
												...
												<td><?php echo $row_inventario['id_inventario']; ?></td>
												<td><?php echo $row_inventario['nombre_producto']; ?></td>
												<td><?php echo $row_inventario['cantidad']; ?></td>
						...
						<div class="modal fade" id="filtroinventario" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<form action="reportes_nph/reporteinventario.php" method="post">
											...
											<div class="form-group">
												...
												<div class="col-md-8">
													<div class="form-group"><label class="small mb-1" for="inputprimernombre"><b>Ingrese valores de filtraciòn</b> </label><input class="form-control py-4" id="filtroempleados" type="text" name="filtroinventario" placeholder="Ingrese una coincidencia" maxlength="" /></div>
												</div>
						...
						cantidad_ver
											...
											<div class="form-group">
												<label class="small mb-1" for="justificacion">Producto disponible</label>
												<textarea class="form-control" id="producto_ver" type="" name="justificacion" value="" placeholder="Ingrese justificacion" disabled required></textarea>
											</div>

											<div class="form-group">
												<label class="small mb-1" for="proveedor_ver">Cantidad</label>
												<input class="form-control" id="cantidad_ver" type="text" name="" value="" required disabled />
											</div>
											...
											<!-- TABLA -->
											<div class="form-group">
												<label class="small mb-1" for="detalle">Movimiestos de producto</label>
												<div class="table-responsive">
									...
									<div class="modal-body">
										<form action="modelos/crud_hogares.php?op=editar" method="post">
											...
											<div class="form-group">
												...
												<input class="form-control" id="hogar" type="text" name="hogar" value="" placeholder="Ingrese nombre del Hogar" maxlength="30" required />
											</div>


											<div class="form-group">
												<label class="small mb-1" for="descripcion">Direccion</label>
												<input class="form-control" id="direccion" type="text" name="direccion" value="" placeholder="Ingrese direccion" maxlength="30" required />
											</div>
											<div class="col-md-6">
												<div class="form-group"><label class="small mb-1" for="inputnumerodetelefono"><b>Número De Teléfono</b> *Solo numeros</label>
													<input class="form-control py-4" id="telefono" type="text" name="telefono" onkeypress="return validarNumero(event);" placeholder="0000-0000" minlength="9" maxlength="9" min="0" required />
												</div>
											</div>
				...
				<script>
					$('.btnVer').on('click', function() {
						...
						*/
						...
						$('#id2_ver').val(datos[1]);
						$('#producto_ver').val(datos[2]);
						$('#cantidad_ver').val(datos[3]);

						// Cantidad_Producto cod_empleado Fecha_Hora Tipo_Movimiento
						
						var id_solicitud = datos[1];
						var url_cargar_detalles = 'modelos/crud_compras.php?op=cargar2&id=' +id_solicitud;
						...
						$.ajax({
							...
							success: function(response) {
								...
								for (let i = 0; i < $data.length; i++) {
									var Cantidad_Producto = $data[i].Cantidad_Producto;
									var cod_empleado = $data[i].cod_empleado;
									var Fecha_Hora = $data[i].Fecha_Hora;
									var Tipo_Movimiento = $data[i].Tipo_Movimiento;

									valores += '<tr>' +
										'<td>' +
										'<input  type="hidden" id="producto' + i + '" name="producto' + i + '" value="' + Cantidad_Producto + '">' +
										'<label for="">' + Cantidad_Producto + '</label>' +
										'</td>' +

										'<td>' +
										'<input  type="hidden" id="cantidad' + i + '" name="cantidad' + i + '" value="' + cod_empleado + '">' +
										'<label for="">' + cod_empleado + '</label>' +
										'</td>' +

										'<td>' +
										'<input  type="hidden" id="cantidad' + i + '" name="cantidad' + i + '" value="' + Fecha_Hora + '">' +
										'<label for="">' + Fecha_Hora + '</label>' +
										'</td>' +

										'<td>' +
										'<input  type="hidden" id="detalle_precio' + i + '" name="precio' + i + '" value=" ' + Tipo_Movimiento + '">' +
